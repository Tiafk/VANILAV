  ...
  function getAdaptiveSizes() {
    ...
    if (containerWidth <= 1400) {
      ...
      const slideWidth = Math.max(
        235,
        Math.min(471, containerWidth * 0.5 - 30),
      );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (topSlider && bottomSlider) {
    const topSliderInstance = createMarqueeSlider(
      topSlider,
      topSliderImages,
      "right",
    );
    const bottomSliderInstance = createMarqueeSlider(
      bottomSlider,
      bottomSliderImages,
      "left",
    );
...
document.addEventListener(
  ...
  function (e) {
    if (e.target.tagName === "IMG") {
      ...
      e.target.innerHTML =
        '<span style="color: #999;">Изображение не загружено</span>';
    }
  },
  true,
);
